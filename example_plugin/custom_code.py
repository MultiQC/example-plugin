...
def example_plugin_execution_start():
    ...
    if "my_example/key_value_pairs" not in config.sp:
        config.update_dict(
            config.sp, {"my_example/key_value_pairs": {"fn": "my_plugin_output.tsv"}}
        )
    if "my_example/mzml" not in config.sp:
        config.update_dict(config.sp, {"my_example/mzml": {"fn": "*.mzml.csv"}})
    if "my_example/search" not in config.sp:
        config.update_dict(config.sp, {"my_example/search": {"fn": "*.search.csv"}})
    if "my_example/fdr" not in config.sp:
        config.update_dict(config.sp, {"my_example/fdr": {"fn": "*.fdr.csv"}})
    if "my_example/plot_data" not in config.sp:
        config.update_dict(
            config.sp, {"my_example/plot_data": {"fn": "my_plugin_plotdata.tsv"}}
        )

    # Some additional filename cleaning
    config.fn_clean_exts.extend([".my_tool_extension", ".removeMetoo"])
    config.fn_clean_exts.extend([".mzml", ".search", ".fdr"])
